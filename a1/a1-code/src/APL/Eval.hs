  ...
  case eval env initial of
    ...
    Right v -> 
      case eval env bound of
        Left err -> Left err
        Right (ValBool _) -> Left "Non-integer loop bound"
        Right (ValInt n) ->
              let loop k pVal =
                    if k >= n 
                      then Right pVal
                      else
                        let envK = envExtend i (ValInt k) env
                            envP = envExtend p pVal envK
                        in case eval envP body of
                          Left err -> Left err
                          Right newP -> loop (k + 1) newP
              in loop 0 v
eval env (Lambda v e) = Right (ValFun env v e)
eval env (Apply fExp aExp) =
  case eval env fExp of 
    Left err -> Left err
    Right (ValInt _) -> Left "Error: apply evaluated to ValInt"
    Right (ValBool _) -> Left "Error: apply evaluated to ValBool"
    Right (ValFun env' vName body) -> -- must evaluate to ValFun
      case eval env aExp of
        Left err -> Left err
        Right aVal -> -- cover any cases of Val
          let env'' = envExtend vName aVal env' --bind parameter name to argument value
          in eval env'' body -- evaluate body using this env
